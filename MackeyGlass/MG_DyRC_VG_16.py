...
Computes the random ER reservoir and a DyRC-VG 16 for each implementation. 
...
def DyRC_VG_skip(num_nodes, x_train, y_train, x_vg, x_test, y_test,
            **kwargs):

    """ 
    Compute random reservoir, DyRC for a given reservoir size.
    ...
    vis_graph = nx.visibility_graph(np.squeeze(x_vg[:,vg_start_point:vg_start_point+num_nodes*skip:skip,0]))
    ...
    pos = axs[1].imshow(new_adj_matrix)
    for ii in range(2):
        axs[ii].set_xticks([])
        axs[ii].set_yticks([])
    ...
    fig, axs = plt.subplots(2, 2, sharex=True)  # share x-axis for all subplots
...
if __name__ == "__main__":
    ...
    # generate training data (integrate mg for some time)
    data = np.load(os.path.join(os.getcwd(),data_name,'mg_data.npy'))
    print(np.shape(data))
    t = np.load(os.path.join(os.getcwd(),data_name,'mg_time.npy'))
    dt = t[1] - t[0]
    ...
    ### DEFINE SKIP AND NUM_NODES HERE ###
    ...
    idx_vg = skip*np.max(num_nodes_)*3
    x_vg = x[:, :idx_vg, :]

    test_length = 6562
    x_test = x[:, idx_vg:idx_vg+test_length, :]
    y_test = y[:, idx_vg:idx_vg+test_length, :]
    print(f"x_train shape: {x_train.shape}, y_train shape: {y_train.shape}")
    print(f"VG data shape: {x_vg.shape}")
    ...
    for num_nodes in num_nodes_:
        ...
        vg_start_points = np.linspace(0, np.shape(x_vg)[1]-num_nodes*skip, num_implementations)
        ...
        for i in range(num_implementations):
            ...
            metrics = DyRC_VG_skip(num_nodes,
                        x_train,
                        y_train,
                        x_vg,
                        x_test,
