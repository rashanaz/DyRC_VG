...
def figure_2b_two_datasets(data_path_lorenz, data_path_mg,
                           ylim_lorenz=(0, 0.01), ylim_mg=(0, 0.02)):
    fig, axs = plt.subplots(2, 1, figsize=(8.5, 8), sharex=True)

    def plot_mae(ax, data_path, ylabel, ylim):
        maes_rand = []
        maes_vg = []
        maes_dens = []
        maes_vg_skip = []
        ...
        width = 0.14
        offset = [-0.35, -0.18, 0, 0.18, 0.35]
        colors = [cpsme_red, cpsme_blue_2, cpsme_blue_3, cpsme_green, cpsme_mint]
        data_list = [maes_rand, maes_vg, maes_dens, maes_vg_skip, maes_sparse]

        for pos_offset, col, data_vals in zip(offset, colors, data_list):
            ax.boxplot(data_vals,
                       ...
                       flierprops=dict(color=col, markeredgecolor=col, markersize=3),
    ...
    # Plot for Lorenz (top)
    plot_mae(axs[0], data_path_lorenz, 'MAE, Lorenz data', ylim_lorenz)

    # Plot for Mackey–Glass (bottom)
    plot_mae(axs[1], data_path_mg, 'MAE, Mackey–Glass data', ylim_mg)
    ...
    # Put stacked legend in top right corner of figure
    fig.legend(handles=[red_patch, blue_2_patch, blue_3_patch, green_patch, mint_patch],
               loc='upper right', bbox_to_anchor=(0.87, 0.98), ncol=1, frameon=True)

    plt.tight_layout(rect=[0, 0, 0.88, 1])  # leave space for legend
    export_figure(fig,
                  name=f'figure_5_combined_2b.png',
                  height=6.5,
                  width=12.5,
                  resolution=300)
    # plt.show()
...
# Example usage:
figure_2b_two_datasets('lorenz_data_1', 'mg_data_1',
                       ylim_lorenz=(0, 0.01),
                       ylim_mg=(0, 0.02))
